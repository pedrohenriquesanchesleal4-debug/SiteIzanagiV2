  ...
  return (
    <section className="relative bg-zinc-950 px-6 py-28 sm:px-12">
      <div className="mx-auto max-w-3xl">
          ...
          <ul className="mt-10 divide-y divide-zinc-800 rounded-2xl border border-zinc-800">
            {commits.map((c) => (
              <li key={c.sha} className="flex items-center gap-4 px-5 py-4">
                <code className="font-mono text-xs text-cyan-400">{c.sha}</code>
        ...
        <Link
          href="/changelog"
          className="mt-8 inline-flex items-center gap-2 font-mono text-sm text-zinc-200 underline decoration-zinc-700 underline-offset-4 transition hover:decoration-cyan-400"
        >
          {t("cta")} →
        </Link>
      </div>
